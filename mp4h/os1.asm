...
TMDR_INIT       .FILL x1000     ; Timer interrupt every 4096 instructions
...
TASK0_INIT
    ...
    LD      R6, TASK0_SP
    LD      R7, TASK0_PC
    RET                         ; Jump to task 0 start
TASK0_SP    .FILL    x4000
TASK0_PC    .FILL    x3000


; TCB must be in user space because TRAPs execute in user mode on 2nd ed. LC3.
CUR_TASK    .FILL    x5000      ; Current task (points to start of TCB)
TCB_END     .FILL    x500C      ; Pointer to where next TCB entry would go

; Keep track of how many times TRAP_YIELD has been called
TRAP_YIELD_CURR     .FILL  x0000      ; Current count of TRAP_YIELD calls
TRAP_YIELD_END      .FILL  x0200      ; Number of TRAP_YIELD executions until halt
TRAP_R0_STORE       .FILL  x0000      ; Store R0 before checking TRAP_YIELD calls
TRAP_R1_STORE       .FILL  x0000      ; Store R1 before checking TRAP_YIELD calls
TRAP_R7_STORE       .FILL  x0000      ; Store R7 before checking TRAP_YIELD calls

CHECK_DONE
    ST      R0, TRAP_R0_STORE   ; Save R0
    LD      R0, TRAP_YIELD_CURR ; Load current TRAP_YIELD counter
    ADD     R0, R0, #1          ; Increment current counter
    ST      R1, TRAP_R1_STORE   ; Save R1
    LD      R1, TRAP_YIELD_END  ; Load max TRAP_YIELD executions
    NOT     R1, R1              ; Negate R0
    ADD     R1, R1, #1          ;
    ADD     R1, R1, R0          ; Compare R1 and R0
    BRzp    FINISH              ; If at or above TRAP_YIELD count, halt
    ST      R0, TRAP_YIELD_CURR ; Store new current counter
    LD      R0, TRAP_R0_STORE   ; Restore R0
    LD      R1, TRAP_R1_STORE   ; Restore R1
    RET

FINISH
    LD      R0, TRAP_R0_STORE  ; Restore R0
    LD      R1, TRAP_R1_STORE  ; Restore R1
    LD      R7, TRAP_R7_STORE  ; Restore R7
    BRnzp   TRAP_HALT          ; Halt the program

TRAP_YIELD
    ; The following code helps halt the program. Do not modify.
    ST      R7, TRAP_R7_STORE
    JSR     CHECK_DONE          ; Checks if TRAP_YIELD called TRAP_YIELD_END number of times
    LD      R7, TRAP_R7_STORE

    ;;; IMPLEMENT ME ;;;;
    ; SAVE R7 onto the task stack and load R7 to the Current task

    ; Save current task's registers & PC to TCB


    ; Done saving current task context. Now switch to next task.


    ; Load next task's registers & PC from TCB


    ; Transfer control to new task
